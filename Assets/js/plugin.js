...
(function($){
    $.fn.myplugin = function (options) {
        ...
        let init = function(board) {
            ...
            let standard = $('<button id="standard">').text('Standard 6x7').css({'margin': '10px', 'display': 'flex','align-items': 'center'});
            let medium = $('<button id="medium">').text('Medium 8x10').css({'margin': '10px', 'display': 'flex','align-items': 'center'});
            let large = $('<button id="large">').text('Large 12x12').css({'margin': '10px', 'display': 'flex','align-items': 'center'});
            let big = $('<button id="big">').text('Big 14x14').css({'margin': '10px', 'display': 'flex','align-items': 'center'});
            let title = $('<h1>').text('My power 4').css({'text-align': 'center', 'color': 'grey','font-family':'Orbitron, sans-serif','font-weight': 'bold'});
            let blockButtons = $('<p id="blockButtons">').css({'margin': '10px', 'display': 'flex','align-items': 'center'});
            let blockChoices = $('<div id="blockChoices">').text('Choose your grid').css({'margin': '10px', 'display': 'flex','align-items': 'center', 'font-family': 'Lucida Console, Monaco, monospace','font-weight': 'bold'});
            ...
            blockButtons.prepend(big);
            blockButtons.prepend(large);
            blockButtons.prepend(medium);
            blockButtons.prepend(standard);
            container.prepend(blockButtons);
            container.prepend(blockChoices);
            container.prepend(title);
            container.css({'display': 'flex', 'flex-direction': 'column', 'align-items': 'center'});
            
            colorPlayer1.change(function(event) {
                if(gameStarted || event.target.value === settings.player2){
                    $(this).val($(this).data('current'));
                    return false;
                }
                
                $(this).data('current', event.target.value);
                settings.player1 = event.target.value;
            });
            
            colorPlayer2.change(function(event) {
                if(gameStarted || event.target.value === settings.player1){
                    $(this).val($(this).data('current'));
                    return false;
                }
                
                $(this).data('current', event.target.value);
                settings.player2 = event.target.value;
            });
            ...
            standard.click(function(e){
                e.preventDefault();
                $('body').myplugin({sizeDefault:[6,7]});
            });
            medium.click(function(e){
                e.preventDefault();
                $('body').myplugin({sizeDefault:[8,10]});
            });
            large.click(function(e){
                e.preventDefault();
                $('body').myplugin({sizeDefault:[12,12]});
            });
            big.click(function(e){
                e.preventDefault();
                $('body').myplugin({sizeDefault:[14,14]});
            });
        ...
        let cellClicked = function(selectedCell) {
            ...
            if(positionY !== null) {
                ...
                if(finish) {
                    ...
                    if(confirm(`Player ${winner} a gagné ! Voulez vous rejouer ?`) === true){
